...
// Paginated read. In a real Prisma-backed store this becomes a `findMany({ skip, take })` plus a
// `count()` — the slicing belongs here at the data layer, not in the route, so the route never has to
// know how the store pages. `offset`/`limit` are assumed already validated by the caller.
