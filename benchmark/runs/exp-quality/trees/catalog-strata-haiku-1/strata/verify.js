...
(async () => {
  ...
  await check('/items paginates with hasMore + nextCursor', async () => {
    const r = await get('/items?limit=2');
  ...
  await check('/items walks pages by cursor without repeating a row', async () => {
    const KEY = 'id';   // NOT always "id" — Mongoose keys on _id
    const p1 = await get('/items?limit=2');
    if (!p1.body.pagination.nextCursor) return;   // fewer than 2 rows in the store; nothing to walk
    const p2 = await get('/items?limit=2&cursor=' + encodeURIComponent(p1.body.pagination.nextCursor));
  ...
  await check('a sort field that is not allowlisted is REJECTED, not honoured', async () => {
    const r = await get('/items?sort=' + encodeURIComponent('password; DROP TABLE'));
