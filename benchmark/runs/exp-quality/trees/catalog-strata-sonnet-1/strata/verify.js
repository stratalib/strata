...
(async () => {
  ...
  await check('/products?offset= uses offset pagination with a total count', async () => {
    const r = await get('/products?limit=5&offset=5');
    assert(r.status === 200, 'status ' + r.status);
    assert(r.body.pagination.total >= r.body.data.length, 'offset pagination should report a total');
    assert(r.body.pagination.offset === 5, 'offset was not honoured');
  });

  await check('/products?category= filters via the allowlist', async () => {
    const r = await get('/products?category=TOYS&limit=50');
    assert(r.status === 200, 'status ' + r.status);
    assert(r.body.data.length > 0, 'expected at least one TOYS product in the seed data');
    assert(r.body.data.every(function (p) { return p.category === 'TOYS'; }),
      'a non-TOYS row leaked through the category filter');
  });
