...
(async () => {
  ...
  await check('/products offset pagination returns a page and a total', async () => {
    const r = await get('/products?offset=0&limit=5');
    assert(r.status === 200, 'status ' + r.status);
    assert(r.body.data.length === 5, 'expected 5 rows, got ' + r.body.data.length);
    assert(typeof r.body.pagination.total === 'number', 'pagination.total missing');
  });
  ...
  await check('filtering by an allowlisted field narrows the results', async () => {
    const r = await get('/products?category=BOOKS&limit=100');
    assert(r.status === 200, 'status ' + r.status);
    assert(r.body.data.length > 0, 'expected at least one BOOKS product');
    assert(r.body.data.every(function (p) { return p.category === 'BOOKS'; }),
      'a non-BOOKS product leaked through the filter');
  });
