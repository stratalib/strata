...
async function startServer() {
  ...
  for (const slot of ["STRIPE_WEBHOOK_SECRET"]) {
...
(async () => {
  ...
  await check('/webhooks/stripe refuses an unsigned webhook', async () => {
    const raw = whEvent('evt_unsigned');
    const r = await request('POST', '/webhooks/stripe', raw, 'application/json');
  ...
  await check('/webhooks/stripe refuses a forged signature', async () => {
  ...
  await check('/webhooks/stripe refuses a replayed timestamp', async () => {
  ...
  await check('/webhooks/stripe accepts a correctly signed event', async () => {
    const raw = whEvent('evt_ok_' + Date.now());
    const r = await request('POST', '/webhooks/stripe', raw, 'application/json', whSign(raw, nowSec()));
  ...
  await check('/webhooks/stripe handles a redelivered event once', async () => {
    ...
    const a = await request('POST', '/webhooks/stripe', raw, 'application/json', whSign(raw, nowSec()));
    const b = await request('POST', '/webhooks/stripe', raw, 'application/json', whSign(raw, nowSec()));
