# Benchmark archive — the 60-run five-arm battery

> **Superseded.** This battery ran on an **earlier prompt instrument** than the one behind
> [`BENCHMARK.md`](../BENCHMARK.md). Its numbers are not comparable with the v1.1 board and must never
> be averaged with it. It is kept because the measurements are real and the method notes below still
> describe how every Strata benchmark is run.
...
The headline it produced: **a cheap model with Strata (92.1%) beat a frontier model without it
(83.9%)**, at a fifth of the cost per run. The premium it also produced — +26% on haiku, +51% on
sonnet — is what v1.1 was built to remove, and did.
